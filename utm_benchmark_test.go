...
func BenchmarkToLatLon(b *testing.B) {
	var err error
	for i := 0; i < b.N; i++ {
		if _, _, err = ToLatLon(coordinate.Easting, coordinate.Northing, coordinate.ZoneNumber, coordinate.ZoneLetter); err != nil {
...
func BenchmarkToLatLonWithNorthern(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		if _, _, err = ToLatLon(coordinate.Easting, coordinate.Northing, coordinate.ZoneNumber, coordinate.ZoneLetter, true); err != nil {
...
func BenchmarkFromLatLon(b *testing.B) {
	var err error
	for i := 0; i < b.N; i++ {
		if _, _, _, _, err = FromLatLon(latLon.Latitude, latLon.Longitude, false); err != nil {
